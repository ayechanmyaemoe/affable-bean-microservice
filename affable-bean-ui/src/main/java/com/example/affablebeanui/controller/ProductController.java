...
import com.example.affablebeanui.model.CartItems;
...
//                "john@gmail.com", "mary@gmail.com", total);
//
...
//    }
    ...
    public String transport() {
        ResponseEntity responseEntity = productService.saveCartItems();
        if(responseEntity.getStatusCode().is2xxSuccessful()) {
            return "redirect:/";
        }
        return "redirect:/ui/checkout-view";
    ...
    public String checkoutTransfer(@ModelAttribute("total") double total, RedirectAttributes redirectAttributes){
        var responseEntity = productService.transfer("john@gmail.com","mary@gmail.com", total);
        if (responseEntity){
            cartService.clearCart();
            return "redirect:/ui/?transfer=true";
        }
        return "redirect:/ui/checkout-view?transferError=true";
    }
    ...
    public String toCheckoutView(Model model, HttpServletRequest request) {
        boolean transferError = Boolean.valueOf(request.getParameter("transferError"));
    ...
    @ResponseBody
    public List<Product> showAllByCategory(@PathVariable int id) {
